...
const StudentList = () => {
  ...
  const newStatus = student.status === "Completed" ? "Not Completed" : "Completed";
  dispatch(updateData({ id: student.id, newData: { ...student, status: newStatus } }));
};

  // Distinct course list for filter
  const courseOptions = ["All Courses", ...new Set(students.map((s) => s.course))];
...
<div className="flex items-center bg-gradient-to-r from-teal-500 to-teal-600 rounded-xl px-3 shadow-md">
  ...
  >
    {[
      "All Courses",
      ...Array.from(new Set(students.map((s) => s.course))).sort((a, b) =>
        a.localeCompare(b)
      ),
    ].map((course, idx) => (
            ...
            <table className="min-w-full border-collapse border border-white/20 bg-white/10 text-white rounded-xl">
              <thead className="bg-gradient-to-r from-indigo-600 to-purple-600">
                <tr>
                  ...
                  <th className="p-3 border border-white/20 text-center">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-white/20">
                    <td className="p-3 border border-white/10">{student.roll}</td>
                    <td className="p-3 border border-white/10">{student.name}</td>
                    <td className="p-3 border border-white/10">{student.course}</td>
                    <td className="p-3 border border-white/10">{student.joiningDate}</td>
                    <td
...
>
  {student.status === "Completed" ? (
    ...
    <span className="text-red-400 font-bold">❌ Not Completed</span>
  )}
</td>
